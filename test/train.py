...
def train_epoch(model, dataloader, optimizer, scheduler, device, config, log_file=None, tb_writer=None, epoch=0, pipeline_type="GeoRasterRenderingPipeline"):
    """Train for one epoch"""
    model.model.train()
    total_loss = 0.0
    num_batches = 0
    ...
    for batch_idx, data in enumerate(progress_bar):
        ...
        if pipeline_type == "RenderFormerRenderingPipeline":
        ...
        try:
            ...
            if pipeline_type == "GeoRasterRenderingPipeline":
                rendered_imgs = model(
                    triangles=triangles,
                    mask=mask,
                    vn=vn,
            ...
            elif pipeline_type == "RenderFormerRenderingPipeline":
                rendered_imgs = model(
                    triangles=triangles,
                    texture=texture,
                    mask=mask,
            ...
            if tb_writer:
                ...
                if global_step % 20 == 0 and gt_images is not None:
...
def validate(model, dataloader, device, config, pipeline_type="GeoRasterRenderingPipeline"):
    """Validate the model"""
    model.model.eval()
    total_loss = 0.0
    num_batches = 0
...
def load_optimizer_state(optimizer, checkpoint_path, update_lr=1e-4):
    ...
    if os.path.exists(training_state_path):
        ...
        print(f"Updating learning rate to {update_lr}")
...
def main():
    ...
    if args.resume_from:
        ...
        pipeline = PIPELINE_CLASS.from_pretrained(args.model_id)
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train
        train_loss = train_epoch(pipeline, train_dataloader, optimizer, scheduler, device, args, None, tb_writer, epoch, args.pipeline_type)
        ...
        if val_dataloader is not None:
            val_loss = validate(pipeline, val_dataloader, device, args, args.pipeline_type)
