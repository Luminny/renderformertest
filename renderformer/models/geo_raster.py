...
class GeoRaster(nn.Module, PyTorchModelHubMixin):
    ...
    def construct_seq(self, tri_vpos_list, valid_mask, vns):
        """
        ...
        if self.config.pe_type == 'nerf':
            ...
            tri_emb = self.tri_encoding_norm(self.tri_encoding_proj(tri_vpos_pe)) + self.tri_token + vn_emb
            tokens.append(tri_emb)
        elif self.config.pe_type == 'rope':
            tri_emb = self.tri_token + vn_emb
            tokens.append(tri_emb)
        else:
            raise ValueError(f"Invalid positional encoding type: {self.config.pe_type}")
    ...
    def forward(self, tri_vpos_list, valid_mask, vns, rays_o, rays_d, tri_vpos_view_tf, tf32_view_tf=False):
        """
        ...
        """
        seq, valid_mask_padded, tri_vpos_list = self.construct_seq(tri_vpos_list, valid_mask, vns)
