...
def main():
    ...
    parser.add_argument("--model_id", type=str, help="Model ID on Hugging Face or local path", default="./checkpoints/checkpoint_epoch_1")
    ...
    for i in range(nv):
        ...
        hdr_img = rendered_imgs[0, i].cpu().detach().numpy().astype(np.float32)
        if args.tone_mapper != 'none':
