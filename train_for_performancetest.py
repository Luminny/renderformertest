"""
Usage:
    python tools/s3_tools.py --download --s3-path "renderformer/traindata/tri1024/000-000.zip" [local train data dir]
    CUDA_VISIBLE_DEVICES=4,5,6,7 python train_for_performancetest.py --train_data_dir [local train data dir] --pretrained  --precision fp32 --batch_size 128
...
class RenderFormerDataset(Dataset):
    def __init__(self, data_dir, resolution=256, max_num_tris=1024):
...
def train_epoch(model, dataloader, optimizer, scheduler, device, config, tb_writer=None, epoch=0):
    """Train for one epoch"""
    model.model.train()
    total_loss = 0.0
    num_batches = 0
...
def main():
    ...
    parser.add_argument("--max_num_tris", type=int, default=1024, 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Train
        train_loss = train_epoch(pipeline, train_dataloader, optimizer, scheduler, device, args, tb_writer, epoch)
